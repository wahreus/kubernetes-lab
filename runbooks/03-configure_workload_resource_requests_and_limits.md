...
Store the running Pod's name in a variable:
...
Inspect the same settings in a human-readable form:

    kubectl describe pod "$POD_NAME" -n resources

Find the `Requests` and `Limits` entries under the `web` container.

## 5. Inspect the scheduling result

Retrieve the node that runs the Pod:
...
Check the Pod's quality-of-service class:
...
Retrieve the replacement Pod's name:
...
Check the quality-of-service class again:
...
Delete the namespace and all resources created by this runbook:

    kubectl delete namespace resources

Verify that the namespace has been removed:

    kubectl get namespace resources

The command should report that the namespace was not found.
